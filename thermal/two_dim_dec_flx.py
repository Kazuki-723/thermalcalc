...
Al_inner_radius = 26e-3  # 52mm の直径 -> 半径
Al_outer_radius = 31e-3  # 62mm の直径 -> 半径
insulation_thickness = 1e-3  # 2mm の直径 -> 半径
...
#計算時間の設定
time = 8.2 #計算したい時間を入れる

# PMMAの減少速度
PMMA_reduction_rate = 0.82e-3  # 1.64mm/sec(直径,FLXシミュより)
...
# グラフの描画（別タブに表示）
ani = animation.ArtistAnimation(fig, ims, interval=(1000*dt), blit=True,repeat_delay=1000)
